...
def create_bd():
    conn = sqlite3.connect("ITea.db")  # или :memory: чтобы сохранить в RAM
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE Points(id INTEGER PRIMARY KEY AUTOINCREMENT, lng text, lat text, tag text)")
    return cursor
...
def insert():
    ...
    answer = "INSERT INTO `Points` (`id`, `tag`, `lat`, `lng`) VALUES (NULL, '" + tag + "', '" + lat + "', '" + lng + "');"
    cursor.execute(
       "INSERT INTO `Points` (`id`, `tag`, `lat`, `lng`) VALUES (NULL, '" + tag + "', '" + lat + "', '" + lng + "');")
    return print(answer)
...
def getValue():
    ...
    return print(final_result , "QQQQQQQQQQQQQQQQQQQQQQQQQ")
if __name__ == '__main__':
